...
export default function BottomNav({ onLinkClick }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-40 bg-white/85 backdrop-blur-md border border-white/50 shadow-3d rounded-2xl md:hidden pb-safe overflow-hidden">
      <div className="flex items-center justify-around h-16 px-1.5 max-w-md mx-auto">
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative ${
            activeSection === '#hero' ? 'text-brand-teal' : 'text-slate-450 hover:text-slate-600'
          }`}
        ...
        >
          ...
          {activeSection === '#hero' && (
            <span className="absolute top-0 w-8 h-0.5 rounded-full bg-brand-teal shadow-[0_0_10px_#0284c7] animate-fade-in" />
          )}
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative ${
            activeSection === '#products' ? 'text-brand-teal' : 'text-slate-450 hover:text-slate-600'
          }`}
          style={{ minWidth: '48px', minHeight: '48px' }}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative ${
            activeSection === '#b2b' ? 'text-brand-teal' : 'text-slate-450 hover:text-slate-600'
          }`}
          style={{ minWidth: '48px', minHeight: '48px' }}
        >
          <Building2 className="w-5 h-5 transition-transform duration-300" />
          <span className="text-[10px] font-bold tracking-wide">B2B</span>
          {activeSection === '#b2b' && (
            <span className="absolute top-0 w-8 h-0.5 rounded-full bg-brand-teal shadow-[0_0_10px_#0284c7] animate-fade-in" />
          )}
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative ${
            activeSection === '#about' ? 'text-brand-teal' : 'text-slate-450 hover:text-slate-600'
          }`}
          style={{ minWidth: '48px', minHeight: '48px' }}
        >
          <Info className="w-5 h-5 transition-transform duration-300" />
          <span className="text-[10px] font-bold tracking-wide">About Us</span>
          {activeSection === '#about' && (
            <span className="absolute top-0 w-8 h-0.5 rounded-full bg-brand-teal shadow-[0_0_10px_#0284c7] animate-fade-in" />
          )}
        </button>
        ...
        <div className="flex items-center justify-center flex-1 h-full">
          <button
            ...
            className="flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full shadow-[0_4px_12px_rgba(16,185,129,0.3)] active:scale-95 transition-transform duration-300 relative border-0 cursor-pointer"
