...
export default function BottomNav({ onLinkClick }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-5 left-4 right-4 z-40 bg-slate-950/92 backdrop-blur-2xl border border-white/10 shadow-[0_12px_40px_rgba(0,0,0,0.65)] rounded-2xl md:hidden pb-safe overflow-hidden">
      <div className="flex items-center justify-around h-16.5 px-2 max-w-md mx-auto">
        
        {/* Home */}
        <button
          onClick={() => handleTabClick('#hero')}
          className="flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative select-none active-scale py-1 cursor-pointer border-0"
          style={{ minWidth: '48px', minHeight: '48px' }}
        >
          <div className={`p-1.5 rounded-xl transition-all duration-300 ${
            activeSection === '#hero' 
              ? 'text-sky-300 bg-sky-500/10 scale-110 shadow-[inset_0_1px_4px_rgba(14,165,233,0.15)]' 
          ...
          <span className={`text-[10px] tracking-wide font-black transition-colors duration-300 ${
            activeSection === '#hero' ? 'text-sky-300 font-extrabold' : 'text-slate-400 font-semibold'
          }`}>Home</span>
          {activeSection === '#hero' && (
            <span className="absolute top-0 w-8 h-1 rounded-full bg-linear-to-r from-brand-teal to-brand-aqua shadow-[0_0_12px_#0284c7] animate-fade-in" />
          )}
        </button>
        ...
        >
          ...
          <span className={`text-[10px] tracking-wide font-black transition-colors duration-300 ${
            activeSection === '#products' ? 'text-sky-300 font-extrabold' : 'text-slate-400 font-semibold'
          }`}>Products</span>
          {activeSection === '#products' && (
            <span className="absolute top-0 w-8 h-1 rounded-full bg-linear-to-r from-brand-teal to-brand-aqua shadow-[0_0_12px_#0284c7] animate-fade-in" />
          )}
        ...
        >
          ...
          <span className={`text-[10px] tracking-wide font-black transition-colors duration-300 ${
            activeSection === '#b2b' ? 'text-sky-300 font-extrabold' : 'text-slate-400 font-semibold'
          }`}>B2B</span>
          {activeSection === '#b2b' && (
            <span className="absolute top-0 w-8 h-1 rounded-full bg-linear-to-r from-brand-teal to-brand-aqua shadow-[0_0_12px_#0284c7] animate-fade-in" />
          )}
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all duration-300 ${
            ...
            <Info className="w-5.5 h-5.5 stroke-[2.2]" />
          </div>
          <span className={`text-[10px] tracking-wide font-black transition-colors duration-300 ${
            activeSection === '#about' ? 'text-sky-300 font-extrabold' : 'text-slate-400 font-semibold'
          }`}>About Us</span>
          {activeSection === '#about' && (
            <span className="absolute top-0 w-8 h-1 rounded-full bg-linear-to-r from-brand-teal to-brand-aqua shadow-[0_0_12px_#0284c7] animate-fade-in" />
          )}
        ...
        <div className="flex items-center justify-center flex-1 h-full">
          <button
            ...
            className="flex items-center justify-center h-11 w-11 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full shadow-[0_4px_16px_rgba(16,185,129,0.45)] active:scale-95 transition-transform duration-300 relative border-0 cursor-pointer"
